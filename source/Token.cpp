...
Token::Token(const std::int64_t number, Allocator& allocator) : m_type(&Type::Number) {
    m_contents = allocator.allocate<RValue>(number);
}

Token::Token(const std::string& label, Allocator& allocator) : m_type(&Type::Label) {
    m_contents = allocator.allocate<std::string>(label);
}
...
RValue* Token::getRValue() const {
    ...
    return std::get<RValue*>(m_contents);
}
